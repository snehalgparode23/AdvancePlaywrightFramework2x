...
Quick reference for querying JSON with the jsonpath-plus package. All examples below run against store.json and are exercised live in jsonpath-queries.e2e.spec.ts.

JSONPath is the JSON analogue of XPath. A path is evaluated against a document and always returns an array of matches (empty array = no match).
...
$..book[*].title → all book titles found via recursive descent 4. Wildcard — _
$.store.book[_].author → authors of all books
$.store.*                 → all direct values of store (book array, bicycle, ...)
$.company.departments._ → every department object
$..book[_] → all book objects anywhere 5. Array index — [n]
...
$.store.book[-2:] → last two books 7. Filtration — [?(...)]
